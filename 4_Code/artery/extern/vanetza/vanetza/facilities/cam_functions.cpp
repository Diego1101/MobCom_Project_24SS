...
void print_indented(std::ostream& os, const asn1::Cam& message, const std::string& indent, unsigned level)
{
    ...
    ReferencePosition_t pos0;
    pos0.longitude = 0;
    pos0.latitude = 0;

    char vehicleIdString[message->ncam.vehicleId.size+1] = {0};
    print_octet_string(message->ncam.vehicleId, vehicleIdString, sizeof(vehicleIdString));

    prefix("Non CAM Parameter") << "\n";
    ++level;
    prefix("Service ID") << message->ncam.serviceId << "\n";
    prefix("Vehicle ID") << vehicleIdString << "\n";
    --level;
...
void print_octet_string(OCTET_STRING_t string, char* buffer, int size)
{
    if(size > string.size)
    {
		uint8_t* ptr = string.buf;
		for(int i = 0; i < string.size; i++)
		{
			buffer[i] = *ptr;
			ptr++;
		}
		buffer[string.size] = 0x0;
    }
}
